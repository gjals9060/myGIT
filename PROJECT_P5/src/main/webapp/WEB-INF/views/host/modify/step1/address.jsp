...
<body>
	...
	<div id="wrap">
		지번/도로명 : <input type="text" id="sample5_address"
			onclick="sample5_execDaumPostcode()" placeholder="주소">
		...
		<form action="facilities" method="post" name="formAddress">
			<input type="hidden" name="address" /> <input type="hidden"
				name="latitude" /> <input type="hidden" name="longitude" />
		</form>
...
<script>
	...
	var markerImage1 = new kakao.maps.MarkerImage(imageSrc1, imageSize1,
			imageOption1), markerPosition1 = new kakao.maps.LatLng(37.537187,
			127.005476); // 마커가 표시될 위치입니다
	...
	var markerImage2 = new kakao.maps.MarkerImage(imageSrc2, imageSize2,
			imageOption2), markerPosition2 = new kakao.maps.LatLng(37.537187,
			127.005476); // 마커가 표시될 위치입니다
					...
					if (status === daum.maps.services.Status.OK) {
						...
						$('#addressInfo').append(
								'주소 : <span id="address1">'
										...
										+ '</span><br />');
						$('#addressInfo').append(
								'상세주소 : <input id="address2" type="text" />');
	...
	function toggleFalse() {
		...
		circle.setPosition(new kakao.maps.LatLng(latlng.getLat(), latlng
				.getLng()));

		// 좌표중심 마커 변경
		marker1.setPosition(new kakao.maps.LatLng(latlng.getLat(), latlng
				.getLng()));
	...
	kakao.maps.event.addListener(map, 'center_changed', function() {
		// 지도 중심좌표를 얻어옵니다 
		var latlng = map.getCenter();
		marker2.setPosition(new kakao.maps.LatLng(latlng.getLat(), latlng
				.getLng()));
			...
			.addListener(
					...
					function() {
						...
						if (toggle) {
							...
							var coord = new kakao.maps.LatLng(latlng.getLat(),
									latlng.getLng());
							...
							geocoder0.coord2Address(coord.getLng(), coord
									.getLat(), callback);
		...
		if (flag) {
			...
			if (!address2) { // 상세주소가 없으면
				...
				$('input[name="address"]').val(
						$('#address1').text() + " " + address2); // 그 뒤에 붙여서
			} // input hidden에 넣어주고
