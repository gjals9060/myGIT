...
<body>
   ...
   <div id="map" style="width: 300px; height: 300px; margin-top: 10px; display: none">
		<button id="focusbtn" style="background-color: #FFFFFF; width: 33px; height: 35px; margin-right: 2px; position: absolute; top: 105px; right: 0; z-index: 5; border: none; border-radius: 10px;" onclick="panTo()">
...
//	marker1.setMap(null); // 지도에 올린다.
	...
	var markerImage2 = new kakao.maps.MarkerImage(imageSrc2, imageSize2, imageOption2),
	    markerPosition2 = new kakao.maps.LatLng(37.5710194900321, 126.992526739499); // 마커가 표시될 위치입니다
...
//   map.setDraggable(true);   //드래그 비활성화
   ...
   function toggleTrue() {
	  // ================ 0511 수정 시작단  by 허민 =================
	  beforePoint = marker2.getPosition();
	  memory = $('input[name="address"]').val();
	  console.log("before : " + beforePoint.toString());
	  console.log("memory : " + memory);
	  $("#focusbtn").prop("disabled", false);
...
*/
   ...
   function toggleFalse() {
	   ...
	   // ================ 0511 수정 시작단  by 허민 =================
		  ...
		  afterPoint = marker2.getPosition();
		  console.log("after : " + afterPoint.toString());

		  if((beforePoint.toString() === afterPoint.toString()) && (memory != "")){
			  console.log("안움직임");
			  $('input[name="address"]').val(memory);
			  $('#addressInfo').empty();
			  $('#addressInfo').append( '확정 주소 : ' + memory);
			  console.log("memory : " + memory);
		  }else{
			  console.log("움직임");
...
*/
   ...
   // 지도 드래그시 주소출력 변경
    kakao.maps.event.addListener(map, 'dragend', function moveMap() {
  ...
  /*  function inputAddress(){
    ...
    function panTo() {
	    ...
	    var moveLatLon = circle.getPosition();
	    
	    var m2 = marker2.getPosition();
	    var c = circle.getPosition();
	    
	    var cen = map.getCenter();
	    console.log("빨간마커위치 : " + moveLatLon);
	    console.log("파란마커위치 : " + m2);
	    console.log("서클위치 : " + c);
	    console.log("지도 중심 위치 : " + cen);
	    ...
	    if(memory != ""){
	    	...
	    	afterPoint = marker2.getPosition();
		    console.log("af파란마커위치 : " + afterPoint);
